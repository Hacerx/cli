...
function extractDefaultValue(node: ts.Node, sourceFile: ts.SourceFile): string | undefined {
  if (ts.isStringLiteral(node)) return `"${node.text}"`;
  if (ts.isNoSubstitutionTemplateLiteral(node)) return node.text;
  if (node.kind === ts.SyntaxKind.TrueKeyword) return 'true';
  if (node.kind === ts.SyntaxKind.FalseKeyword) return 'false';
  if (ts.isNumericLiteral(node)) return node.text;
  if (ts.isCallExpression(node)) return node.getText(sourceFile);
  return undefined;
}
...
function parseFlagObject(
  ...
  const result: Omit<FlagDef, 'type'> = { description: '' };
  ...
  for (const prop of objLiteral.properties) {
    ...
    switch (name) {
      ...
      case 'required':
        ...
        if (init.kind === ts.SyntaxKind.FalseKeyword) result.required = false;
        break;
      case 'defaultValue':
...
function parseCommandSource(filePath: string): Omit<CommandEntry, 'path'> | null {
  ...
  ts.forEachChild(sourceFile, (node) => {
    ...
    if (ts.isVariableStatement(node)) {
      for (const decl of node.declarationList.declarations) {
        if (!ts.isIdentifier(decl.name) || decl.name.text !== 'flags') continue;
...
function collectSourceFiles(
  ...
  const results: Array<{ path: string[]; filePath: string }> = [];
...
function toAnchor(commandPath: string[]): string {
...
function renderFlagsTable(flags: Record<string, FlagDef>): string {
  ...
  if (entries.length === 0) return 'This command has no options.';
...
function generateGroupDoc(group: string, entries: CommandEntry[]): string {
