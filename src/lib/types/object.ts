...
import { format } from "../files.js";
import { PicklistValueSchema, PicklistResult } from "../../types/salesforce.js";
...
export async function generateTypes(
  sObjectDef: DescribeSObjectResult,
): Promise<string> {
...
function additionalFieldInfo(field: Field): string {
  ...
  moreInfo += field.inlineHelpText
    ? `${moreInfoPrefix}@HelpText ${field.inlineHelpText}`
    : "";
  moreInfo += field.calculatedFormula
    ? `${moreInfoPrefix}@Formula - ${field.calculatedFormula}`
    : "";
  ...
  moreInfo += field.relationshipName
    ? `${moreInfoPrefix}@RelationshipName - ${field.relationshipName}`
    : "";
 ...
 * }
 */
function generateChildRelations(object: DescribeSObjectResult): {
  childrenDef: string;
  imports: string;
} {
  let childrenDef = "\n// Child relationships\n";
